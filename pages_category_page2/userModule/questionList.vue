<template>
	<view class="question-box">
		<view class="tabsbox">
			<u-tabs :list="questionTypeList" bar-width="140" :bold="false" active-color="#FF7800" inactive-color="#999999"
		...
		<view v-if="questionTypeFlag == 0">
			<view v-if="problemList.length>0">
				<view class="wid function-box">
					...
					<view v-else class="flex-row-plus editicon-box flex-items fs28" @click="editClick">
						<image class="editicon" src="../../static/images/collection-editicon.png"></image>
						...
						<view class="flex-item" @click="goQuestionDetails(item.productId,item.problemId)">
							<view class="item u-border-bottom wid flex-row-plus flex-items">
								<view v-show="allCheckShow">
									<image v-if="item.selected == 1" @click.stop="problemItemSel(index,0)" src="../../static/images/select-active.png" class="cart-select-img"></image>
									<image v-else @click.stop="problemItemSel(index,1)" src="../../static/images/select-empty.png" class="cart-select-img"></image>
								</view>
								...
								<view class="title-wrap replynum-box">
									...
									<label class="font-color-666 fs24" v-if="item.count>0">{{item.count}}条回复</label>
									<label  class="font-color-666 fs24" v-else>暂无回复</label>
								</view>
							</view>
						</view>
				...
				<view v-show="allCheckShow" class="allcheck-box flex-row-plus flex-sp-between flex-items">
					<view class="left">
						<image v-if="isAllProblemCheck" src="../../static/images/select-active.png" class="cart-select-img" @click="allProblemSel(0)"></image>
						<image v-else src="../../static/images/select-empty.png" class="cart-select-img" @click="allProblemSel(1)"></image>
						<text>全选</text>
					</view>
					<view class="right">
			...
			<view v-else class="mar-top-60 empty-box">
				<image class="question-empty" src="../../static/images/question-empty.png"></image>
		...
		<view v-if="questionTypeFlag == 1">
			<view v-if="answerList.length>0">
						...
						<view class="item u-border-bottom flex-item" @click="goQuestionDetails(item.productId,item.problemId)">
							<view class="flex-row-plus flex-sp-between flex-items-plus wid" style="border-radius: 50%;">
								<view class="flex-items-plus flex-row wid">
									<view v-show="allCheckShow">
										<image v-if="item.selected == 1" @click.stop="answerItemSel(index,0)" src="../../static/images/select-active.png" class="cart-select-img"></image>
										<image v-else @click.stop="answerItemSel(index,1)" src="../../static/images/select-empty.png" class="cart-select-img"></image>
									</view>
									<image class="product-img" mode="aspectFill" :src="item.image" />
									<view class="title-wrap mar-left-20 replynum-box wid">
										...
										<view class="flex-items-plus flex-row flex-sp-between">
											<label v-if="allCheckShow" style="width: 270rpx;" class="font-color-666 fs24 overflow">{{item.name}} : {{item.answer}}</label>
											<label v-else style="width: 360rpx;" class="font-color-666 fs24 overflow">{{item.name}} : {{item.answer}}</label>
											<view class="flex-items-plus">
												<label class="fs24 font-color-999">共{{item.count}}个回答</label>
				...
				<view v-show="allCheckShow" class="allcheck-box flex-row-plus flex-sp-between flex-items">
					<view class="left">
						<image v-if="isAllAnswerCheck" src="../../static/images/select-active.png" class="cart-select-img" @click="allAnswerSel(0)"></image>
						<image v-else src="../../static/images/select-empty.png" class="cart-select-img" @click="allAnswerSel(1)"></image>
						<text>全选</text>
					</view>
					<view class="right">
