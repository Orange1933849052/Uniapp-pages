		    ...
		    <view class="messageBox">
		      <view class="messageType">
		        <view v-if="item.noticeType == 1" class="messageTypeL">
		          <img src="../../static/assets/images/cereshop/tongzhi@2x.png" alt="">
		          <span>订单消息</span>
		        </view>
				<view v-else class="messageTypeL">
				  <img src="../../static/assets/images/cereshop/xitonggonggao@2x.png" alt="">
				  <span>系统公告</span>
				</view>
		        <view class="messageTypeR">{{item.createTime}}</view>
		      </view>
		      <view class="messageInfo mar-left-35 mar-right-35">
				  <rich-text :nodes="formatRichText(item.noticeContent)"></rich-text>
		      </view>
...
export default {
  ...
  methods: {
    ...
    goToMesDetail (noticeId,only,jump) {
	   if(jump == 2){
		   uni.navigateTo({
			   url:"../../pages_category_page1/orderModule/orderDetails?orderId="+only+'&noticeId='+noticeId
		   })
	   }else{
		  uni.navigateTo({
