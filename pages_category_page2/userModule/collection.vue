<template>
	<view class="collection-box">
		<view class="tabsbox">
			<u-tabs :list="collectionTypeList" bar-width="60" :bold="false" active-color="#FF7800" inactive-color="#999999"
		...
		<view v-if="collectionTypeFlag == 0">
			<view v-if="productCollect.length>0">
						...
						<view class="flex-item" @click.stop="toGoodsDetails(item.productId,item.shopId,item.skuId)">
							<view class="item u-border-bottom wid flex-row-plus flex-items">
								<view v-show="allCheckShow">
									<image v-if="item.selected == 1" @click.stop="productItemSel(index,0)" src="../../static/images/select-active.png" class="cart-select-img"></image>
									<image v-else @click.stop="productItemSel(index,1)" src="../../static/images/select-empty.png" class="cart-select-img"></image>
								</view>
				...
				<view v-show="allCheckShow" class="allcheck-box flex-row-plus flex-sp-between flex-items">
					<view class="left">
						<image v-if="isAllProCheck" src="../../static/images/select-active.png" class="cart-select-img" @click="allProductSel(0)"></image>
						<image v-else src="../../static/images/select-empty.png" class="cart-select-img" @click="allProductSel(1)"></image>
						<text>全选</text>
					</view>
					<view class="right">
			...
			<view v-else class="mar-top-60 empty-box">
				<image class="collect-empty" src="../../static/images/collect-empty.png"></image>
		...
		<view v-if="collectionTypeFlag == 1">
			<view v-if="storeCollect.length>0">
						...
						<view class="item u-border-bottom flex-items">
							<view class="flex-row-plus flex-sp-between flex-items-plus wid">
								<view class="flex-items-plus flex-row pad-topbot-10">
									<view v-show="allCheckShow">
										<image v-if="item.selected == 1" @click.stop="storeItemSel(index,0)" src="../../static/images/select-active.png" class="cart-select-img"></image>
										<image v-else @click.stop="storeItemSel(index,1)" src="../../static/images/select-empty.png" class="cart-select-img"></image>
									</view>
									<image class="head-img" mode="aspectFill" :src="item.shopLogo" />
									<!-- 此层wrap在此为必写的，否则可能会出现标题定位错误 -->
									<view class="title-wrap mar-left-20">
										<text class="title u-line-2">{{ item.shopName }}</text>
										<label class="font-color-999 fs24">{{item.person}}人关注</label>
									</view>
								</view>
								<view class="toStore flex-items-plus fs24" @click="toStoreClick(item.shopId)">进入店铺</view>
							</view>
						</view>
					</u-swipe-action>
				</view>
				<view v-show="allCheckShow" class="allcheck-box flex-row-plus flex-sp-between flex-items">
					<view class="left">
						<image v-if="isAllStoreCheck" src="../../static/images/select-active.png" class="cart-select-img" @click="allStoreSel(0)"></image>
						<image v-else src="../../static/images/select-empty.png" class="cart-select-img" @click="allStoreSel(1)"></image>
						<text>全选</text>
					</view>
					<view class="right">
