	...
	<view>
		<view class="totalClient-topBackImg flex-items flex-column">
			...
			<view class="content mar-top-30" v-if="SubordinateSalesData.total>0">
				...
				<view class="flex-center clientList-box mar-top-10" v-for="(item, index) in SubordinateSalesDatalist" :key="index">
		...
		methods: {
			getSubordinateSales() {
				...
				NET.request(API.FindSubordinateSalesList, {
					...
					this.SubordinateSalesDatalist = this.SubordinateSalesDatalist.concat(res.data.list)
...
<style lang="scss">
	...
	.totalClient-topBackImg {
		width: 100%;
		height: 400upx;
		background: url(../../static/images/totalPersonnel-topBackImg.png) no-repeat;
