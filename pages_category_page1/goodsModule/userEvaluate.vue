<template>
	<view class="container">
		...
		<view class="evaluateTitle-box flex-row-plus flex-sp-around">
			<label :class="{'evaluateTitle-on' : evaluateTitleFlag == 1}" @click="evaluateTitleClick(1)">全部评价({{myCommentList.total}}）</label>
			<label :class="{'evaluateTitle-on' : evaluateTitleFlag == 2}" @click="evaluateTitleClick(2)">有图({{myCommentList.imageTotal}}）</label>
		</view>
				...
				<view class="evaluate-contentbox" @click="commentDetails(index)" v-for="(item, index) in commentVOList" :key="index">
					<view class="evaluate-content flex-items flex-row flex-sp-between">
						<view class="flex-items">
							...
							<image class="user-headSmallImg" src="../../static/images/store-logo.png" v-else></image>
					...
					<view class="goodsDes-box flex-column-plus mar-top-30">
						...
						<view class="flex-items flex-row mar-top-30 flex-sp-between">
							...
							<view class="praise-box flex-items flex-row">
								<image class="praise-icon" @click.stop="zanTap(index,item.commentId,0)" src="../../static/images/praiseActive-icon.png"
								 v-if="item.ifLike"></image>
								<image class="praise-icon" @click.stop="zanTap(index,item.commentId,1)" src="../../static/images/praise-icon.png"
								 v-else></image>
	...
	.container {
		...
		.evaluateTitle-box {
			...
			.evaluateTitle-on {
				...
				border-bottom: 2upx solid #FF7800;
			}
		}
