	...
	<view>
		...
		<view> 
			<view class="flex-items-plus flex-row mar-top-20">
				...
				<label class="mar-left-40 fs26" @click="getList">搜索</label>
			</view>
			...
			<view class="historySear-box flex-wrap-1 flex-row">
				<view  v-for="(item, index) in historyList" :key="item.searchId" class="historySearDel-box mar-top-30 flex-items-plus mar-left-30">
					<label @click="historyKeyword(item.search)" class="fs28">{{item.search}}</label>
					<view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			deleteAllSearch(){
				NET.request(API.DeleteAllSearch,{},'DELETE').then(res => {
					this.historyList = res.data
				}).catch(res => {
