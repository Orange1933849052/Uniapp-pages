<template>
  <div class="hom-pro-list warp" :class="'terminal'+terminal" v-if='firstProduct'>
    <div class="title" :class="'text-align-'+componentContent.textAlign">
      ...
      <a class="btn-all a-link" @click="goDiscount(shopId!==0 && {shopId:shopId,id:componentContent.shopDiscountId})">查看全部<i class="iconfont icon-arrow-right"></i></a>
    </div>
    <div v-if="(componentContent.shopDiscountId && discountInfo && count.length > 0) || componentContent.productData.productIdList !==0">
      <div class="discountList" v-if="terminal === 4" :class="this.typeId === 1 && 'pc'">
        ...
        <div class="listRight">
          <div class="listItem" v-for='it in discountInfo.products' :key='it.productId' @click="goGoods(it)">
            <div class="imgBox">
      ...
      <div v-else class="mobileList">
        <div class="topList mItem" @click="goGoods(firstProduct)" v-if='firstProduct'>
          <div class="topImgBg">
            ...
            <div class="leftText">
              <image class="img" src="../../../static/assets/images/icon-discount.png"></image>
              <span>限时折扣</span>
            </div>
    ...
    getData() {
      ...
      if(this.typeId === 1){
        ...
        if(_.componentContent.shopDiscountId){
          ...
          //   title: '加载中...'
          // })
          NET.request1(`${API.getDiscounts}?shopId=${_.shopId}&ids=${_.componentContent.shopDiscountId}`, null, 'GET').then(res => {
			store.commit("loadNumReduce");
